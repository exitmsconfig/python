# python exitmsconfig thon 
[![Build Status](https://travis-ci.org/exitmsconfig/python.svg?branch=master)](https://travis-ci.org/exitmsconfig/python)
...
```
![](https://github.com/exitmsconfig/python/blob/master/img/download.gif)
...
[详相说明相关截图](https://github.com/exitmsconfig/hack.python/blob/master/res/doc/%E9%AA%8C%E8%AF%81%E7%A0%81%E8%AF%86%E5%88%ABv1%2BHTTP%E5%8D%8F%E8%AE%AERange%E7%89%B9%E6%80%A7%E5%88%86%E6%9E%90.pdf)
...
例，红包总金额为10元，分成7个
➜  python git:(master) ✗ py redpackage.py 10 7
...
```
![](https://github.com/exitmsconfig/python/blob/master/img/redpackage.gif)
...
```
![](https://github.com/exitmsconfig/python/blob/master/img/tire.png)

```
fail 指针的理解图解,以下内容需要仔细读
参考：http://www.cnblogs.com/crazyacking/p/4659501.html
```
![](https://github.com/exitmsconfig/python/blob/master/img/ac_fail_pointer.png)
...
```
![](https://github.com/exitmsconfig/python/blob/master/img/cmd.png)
```
测试搜索指定字符串：
...
get help：
➜  python git:(master) ✗ py calc24.py -h
...
exp:
➜  python git:(master) ✗ py calc24.py -n 10,8,9,4
[10, 8, 9, 4]
9 - 10 = -1
4 + -1 = 3
8 * 3 = 24
...
or random test:
➜  python git:(master) ✗ py calc24.py
[9, 10, 3, 6]
10 - 9 = 1
3 + 1 = 4
6 * 4 = 24
...
以利用堆栈结构减少计算机内存访问。
➜  python git:(master) ✗ python rpn.py
...
第三方的gevent为pythonthon提供了比较完善的协程支持。通过greenlet实现协程，其基本思想是：
    当一个greenlet遇到IO操作时，比如访问网络，就自动切换到其他的greenlet，等到IO操作完成，再在适当的时候切换回来继续执行。
    由于IO操作非常耗时，经常使程序处于等待状态，有了gevent为我们自动切换协程，就保证总有greenlet在运行，而不是等待IO。

由于切换是在IO操作时自动完成，所以gevent需要修改pythonthon自带的一些标准库，这一过程在启动时通过monkey patch完成：

依赖：
pip install gevent

执行：
➜  python git:(master) ✗ python coroutine.py
...
```
base64编码原理，使用pythonthon实现base64编码，可能有bug，未完全完善版
1,准备一个包含64个字符的数组
...
例：
➜  python git:(master) ✗ python base64_str.py lock
bG9jaw==
➜  python git:(master) ✗ echo -n lock|base64
bG9jaw==
...
while True:
    try:
    	...
    	print 'Exception:', e
由于没有在__builtins__中引用file和open，所以常规的编码技巧是行不通的。但可以在pythonthon解释器中挖掘出另一种代替file或open引用的方法。

另类读取文件的方式：
().__class__.__bases__[0].__subclasses__()[40]('1.key').read()
这个方法依然可以读取到1.key的内容，coder,hack,geek可以深入了解下，本人测试时的python版本为：pythonthon 2.7.12
```
...
➜  python git:(master) ✗ py avl_tree.py
8
9
1
...
### revert_list.py 反转链表
```
➜  python git:(master) ✗ py revert_list.py
1
2
3
start revert list ...
...
### kmp.py   kmp字符串查找算法
```
➜  python git:(master) ✗ python kmp.py
...
➜  python git:(master) ✗ python compress.py
...
### interpreter.py pythonthon解释器理解
```
pythonthon会执行其他3个步骤：词法分析，语法解析和编译。
...
```
➜  python git:(master) py linked_list.py
...
执行：python knn.py
```
![](https://github.com/exitmsconfig/python/blob/master/img/knn.png)
...
执行：python svm.py
```
![](https://github.com/exitmsconfig/python/blob/master/img/svm.png)


### (前序，中序，后序，层序) btree.py
```
➜  python git:(master) ✗ python btree.py
...
构造树结构如下图
```
![](https://github.com/exitmsconfig/python/blob/master/img/btree.png)
...
```
![](https://github.com/exitmsconfig/python/blob/master/img/crawl_run.gif)
![](https://github.com/exitmsconfig/python/blob/master/img/crawl_db_data.png)
